...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 侯萌i🥝")[0];
}
    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "你已被发现😜",
            message: "小伙子，别借鉴我代码-侯萌（2021.03 v2.0修改版）！",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "warning",
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 侯萌i🥝 %c 你正在访问侯萌i🥝home",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 侯萌i🥝") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('侯萌i🥝的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://cloudcache.tencentcs.cn/qcloud/ui/cloud-community/build/base/images/icon-man_397.svg' title='2024'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://cloudcache.tencentcs.cn/qcloud/ui/cloud-community/build/base/images/icon-menu_000.svg' title='2025~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
<details class="folding-tag" cyan><summary> 设置自定义背景 </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>
